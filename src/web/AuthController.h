...
public:
    ...
    inline void Register(HttpServer& http) {
        auto& s = http.Raw();
...
private:
    static inline void ReplyJsonAuth(httplib::Response& res, const ApiResult& r) {
        auto j = r.ToJson();
        res.set_content(j.dump(), "application/json; charset=utf-8");
    }

    static inline bool ExtractBearerToken(const httplib::Request& req, std::string& token) {
